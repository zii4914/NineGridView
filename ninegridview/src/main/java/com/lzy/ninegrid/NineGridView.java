...
public class NineGridView extends ViewGroup {
  ...
  private static NineGridConfig.ImageLoader mImageLoader;        //全局的图片加载器(必须设置,否者不显示图片)
  ...
  private List<NineGridViewWrapper> mWrappers;
  ...
  public void setAdapter(@NonNull NineGridViewAdapter adapter) {
    ...
    if (mImageInfo == null) {
      for (int i = 0; i < imageCount; i++) {
        NineGridViewWrapper wrapper = getItemView(i);
        if (wrapper == null) return;
        addView(wrapper, generateDefaultLayoutParams());
      }
      setAddView();
    } else {
  ...
  private void setAddView() {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (child instanceof NineGridViewWrapper) {
        NineGridViewWrapper wrapper = (NineGridViewWrapper) child;
        // TODO zii: 2019/7/8 here
  ...
  private NineGridViewWrapper getItemView(final int position) {
    NineGridViewWrapper wrapper;
    if (position < mWrappers.size()) {
      wrapper = mWrappers.get(position);
    } else {
      wrapper = mAdapter.generateItemView(getContext());
  ...
  public static void setImageLoader(NineGridConfig.ImageLoader imageLoader) {
    mImageLoader = imageLoader;
  }

  public static NineGridConfig.ImageLoader getImageLoader() {
    return mImageLoader;
  }
